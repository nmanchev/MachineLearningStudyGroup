...
print("\n")
print("Individual Regression Coefficients t Test:\n")

# Compute sum of squared errors
sse = np.sum( (y_hat(x, w) - y) ** 2)    
print("SSE:", sse)

# Compute sample variance (SSE / degree of freedom)
n = x.shape[0]
sigma = sse / (n - (k+1))
print("sigma:", sigma)

# Covariance matrix
C = sigma * np.linalg.inv(np.dot(x.T, x))
...
p_values = 1 - stats.t.cdf(abs(t0w), float(n - (k+1)))
